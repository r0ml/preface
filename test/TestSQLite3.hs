...
testExec TestEnv{..} = TestCase $ do
  ...
  exec conn " ; ; ; ; ; "
  exec conn "--"
  Left SQLError{sqlError = SQLiteErrorError} <- try $ exec conn "/*"
...
testPrepare TestEnv{..} = TestCase $ do
  ...
  withConn $ \conn -> do
    ...
    withStmt conn
      ...
      return ()
    Left SQLError{sqlError = SQLiteErrorError} <- try $ exec conn "BEGIN"
...
testCloseBusy _ = TestCase $ do
  ...
  Left SQLError{sqlError = SQLiteErrorBusy} <- try $ close conn
  finalize stmt
  close conn
...
testNamedBindParams TestEnv{..} = TestCase $ do
  withConn $ \conn -> do
    withStmt conn "SELECT :foo / :bar" $ \stmt -> do
      ...
      Just fooIdx <- bindParameterIndex stmt ":foo"
      Just barIdx <- bindParameterIndex stmt ":bar"
      -- Test that we get Nothing back for unknown names
      Nothing <- bindParameterIndex stmt "intentionally_undefined"
      Right () <- directBindInt64 stmt fooIdx 4
      Right () <- directBindInt64 stmt barIdx 2
      Row <- step stmt
      1 <- columnCount stmt
    ...
    withStmt conn "SELECT @n1+@n2" $ \stmt -> do
      -- Test that we get something back for known names
      Just _n1 <- bindParameterIndex stmt "@n1"
      Just _n2 <- bindParameterIndex stmt "@n2"
      -- Here's where things get confusing..  You can't mix different
      -- types of :/$/@ parameter conventions.
      Nothing <- bindParameterIndex stmt ":n1"
      Nothing <- bindParameterIndex stmt ":n2"
      return ()
...
testTypedColumns TestEnv{..} = TestCase $ do
  withConn $ \conn -> do
    ...
    withStmt conn "SELECT * FROM foo" $ \stmt -> do
      ...
      Row <- step stmt
      2 <- columnCount stmt
      [SQLInteger 3, SQLInteger 4] <- typedColumns stmt [Just SQLColumnTypeInteger, Just SQLColumnTypeInteger]
      Done <- step stmt
      2 <- columnCount stmt
      return ()
    withStmt conn "SELECT * FROM foo" $ \stmt -> do
      Row <- step stmt
      2 <- columnCount stmt
      [SQLText "1", SQLText "2"] <- typedColumns stmt [Just SQLColumnTypeText, Just SQLColumnTypeText]
      Row <- step stmt
      2 <- columnCount stmt
      [SQLFloat 3.0, SQLFloat 4.0] <- typedColumns stmt [Just SQLColumnTypeFloat, Just SQLColumnTypeFloat]
      Done <- step stmt
      2 <- columnCount stmt
      return ()
  where
...
testErrors TestEnv{..} = TestCase $ do
  withConn $ \conn -> do
    ...
    expectError SQLiteErrorConstraint $
      exec conn "INSERT INTO foo VALUES (3)"
    ...
    withStmt conn "SELECT ?" $ \stmt -> do
      forM_ [-1, 0, 2] $ \i -> do
        expectError SQLiteErrorRange $ bindSQLData stmt i $ SQLInteger 42
        expectError SQLiteErrorRange $ bindSQLData stmt i SQLNull
  ...
  withConnShared $ \conn -> do
    ...
    withStmt conn "SELECT * FROM foo" $ \stmt -> do
      ...
      expectError SQLiteErrorLocked $ exec conn "DROP TABLE foo"
      withConnShared $ \conn -> do
        expectError SQLiteErrorBusy $ exec conn "DROP TABLE foo"
        ...
        expectError SQLiteErrorBusy $ exec conn "COMMIT"
      ...
      assertBool "Step after table vanishes should fail with SQLITE_ERROR or SQLITE_SCHEMA"
                 (err == SQLiteErrorError ||  -- SQLite 3.7.13
                  err == SQLiteErrorSchema)   -- SQLite 3.6.22
...
testDecodeError TestEnv{..} = TestCase $ do
  withStmt conn "SELECT ?" $ \stmt -> do
    Right () <- directBindText stmt 1 invalidUtf8
    Row <- step stmt
  ...
  withConnShared $ \conn -> do
    withStmt conn "SELECT * FROM testDecodeError" $ \stmt -> do
      Row <- step stmt
      SQLColumnTypeText <- columnType stmt 0
      txt <- directColumnText stmt 0
...
{-
    ...
    Left jj <- try $ exec conn "UPDATE tbl SET rowid=4"
...
testGetAutoCommit TestEnv{..} = TestCase $
  withConn $ \conn -> do
    True <- getAutoCommit conn
    exec conn "BEGIN"
    False <- getAutoCommit conn
    Left (SQLiteErrorError, _) <- directExec conn "BEGIN"
    False <- getAutoCommit conn

    exec conn "ROLLBACK"
    True <- getAutoCommit conn
    Left (SQLiteErrorError, _) <- directExec conn "ROLLBACK"
    True <- getAutoCommit conn

    exec conn "BEGIN"
    False <- getAutoCommit conn
    Left (SQLiteErrorFull, _) <- directExec conn
...
testStatementSql TestEnv{..} = TestCase $ do
  ...
  withStmt conn q1 $ \stmt -> do
    Just (Utf8 sql1) <- statementSql stmt
...
testCustomFunction TestEnv{..} = TestCase $ do
  withConn $ \conn -> do
    ...
    Left SQLError{sqlError = SQLiteErrorError} <-
...
testCustomFunctionError TestEnv{..} = TestCase $ do
  withConn $ \conn -> do
    ...
    assertBool "Catch exception"
        (sqlError == SQLiteErrorError && sqlErrorDetails == "error message")
  where
...
testInterrupt TestEnv{..} = TestCase $
  withConn $ \conn -> do
    ...
    Left SQLiteErrorInterrupt <- directStep stmt
    Left SQLiteErrorInterrupt <- directFinalize stmt
...
withTestEnv cb =
  ...
  where
    ...
    withConnPath path cb = do
      conn <- open path
      r <- cb conn `onException` directClose conn
